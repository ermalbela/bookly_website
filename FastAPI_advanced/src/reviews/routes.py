...
@review_router.get('/{review_uid}', response_model=ReviewModel, dependencies=[role_checker])
...
@review_router.get('/get_user_reviews/{user_id}', response_model=List[ReviewModel], dependencies=[role_checker])
async def get_all_user_reviews(user_id: str, session: AsyncSession = Depends(get_session)):
